...
void RecordManager::conditionSelect(char* tableName,int attrno,char* op, Attribute attr,TableInfo tbif,Tuple* tup)//参数：表名，属性名，算数比较符，比较值
{
	tablePage tbpg;
	int pgNum = buffer.getTablePageNum(tableName);
	char* pgdata;
...
vector<Tuple> RecordManager::nonconditionSelect(char* tableName, TableInfo tbif)                     //参数：表名,存放数组，tableinfo；返回全部tuple
{
	vector<Tuple> tup;
	vector<Tuple> temp;
	tablePage tbpg;
