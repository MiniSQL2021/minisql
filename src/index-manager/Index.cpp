...
Index::Index(std::string table_name, Attribute attr) {
...
void Index::createIndex(std::string file_path, int type) {
    ...
    //并且先初始化一颗B+树
    if (type == TYPE_INT) {
        Tree<int> *tree = new Tree<int>(file_path, key_size, degree);
        indexIntMap.insert(intMap::value_type(file_path, tree));
    } else if (type == TYPE_FLOAT) {
        Tree<float> *tree = new Tree<float>(file_path, key_size, degree);
        indexFloatMap.insert(floatMap::value_type(file_path, tree));
    } else {
        Tree <std::string> *tree = new Tree<std::string>(file_path, key_size, degree);
...
void Index::createIndexWithDatas(std::string file_path, int type, int n, vector<Tuple> datasTuple) {
    ...
    for (int i = 0; i < datasTuple.size(); i++) {
        ...
        data.type = type;
        if (type == 0) {
            data.datai = datasTuple[i].attr[n];
        } else if (type == 1) {
            data.dataf = datasTuple[i].attr[n];
        } else {
            data.datas = datasTuple[i].attr[n];
        }
